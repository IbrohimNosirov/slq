...
function qr_tridiag!(diag::AbstractVector{Float64}, subdiag::AbstractVector{Float64}, evec_row::AbstractVector{Float64})
        ...
        for i = 1:itr_max
                ...
                while j <= subdiag_n
                        ...
                        block_end = j
                        @assert block_begin > 0 "invalid matrix index"
                        @assert block_end >= block_begin (block_begin, block_end)
                        if block_end == block_begin
                                cs, sn = eigen_small!(view(diag,    block_begin:block_end),
                                                      view(subdiag, block_begin:block_end),
                                                      view(diag,    block_begin+1:block_end+1))
                                apply_evec_to_evec_row!(evec_row, cs, sn, sn, cs, block_begin)
                        else
                                # add QL here.
                                #chase_bulge_ql!(diag, subdiag, evec_row, block_begin, block_end)
                                chase_bulge_qr!(diag, subdiag, evec_row, block_begin, block_end)
                        end
                        j += 1
                end
        end
...
function chase_bulge_qr!(diag::AbstractVector{T}, subdiag::AbstractVector{T}, evec_row::AbstractVector{T}, idx_begin::Integer, idx_end::Integer) where T <: AbstractFloat
...
# 2x2 matrix case
function apply_evec_to_evec_row!(evec_row::AbstractVector{T}, v1::T, v2::T, v3::T, v4::T, i::Integer) where T <: AbstractFloat
        tau1 = evec_row[i]
